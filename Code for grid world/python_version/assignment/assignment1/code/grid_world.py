...
class GridWorld():
    ...
    def __init__(self, args=None):
        ...
        self.num_states = args.env_size[0] * args.env_size[1]
        self.start_state = args.start_state
        self.target_state = args.target_state
        self.forbidden_states = args.forbidden_states

        self.agent_state = args.start_state
    ...
    def render(self,animation_interval=None):
        ...
        if not self.args.is_save_gif:
            plt.savefig(self.args.results_path+'/policy.png')
        else:
            plt.savefig(self.args.results_path+'/grid_world.png')
        if animation_interval is not None:
            plt.pause(animation_interval)
        else:
            plt.pause(self.args.animation_interval)
        if self.args.debug:
            input('press Enter to continue...')   
        if self.args.is_save_gif:
            img = imageio.imread(self.args.results_path+'/grid_world.png')
            return img
        else:
            return None
